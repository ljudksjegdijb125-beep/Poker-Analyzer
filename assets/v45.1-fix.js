...
(function(){
  ...
  function updateReplyControls(){
    ...
    if(send&&!busy){send.title=mode==='manual'?'发送消息，不自动回复':mode==='debounce'?'发送后等待短暂安静再回复':'发送并立即回复'}
  ...
  function showPendingStatus(chatId){
    ...
    else setGenerationState?.('typing','正在等待连续输入…');
    updateReplyControls();
  }
  ...
  window.saveReplyTriggerSetting=function(){
    ...
    data.settings.replyTriggerMode=validTrigger.has(value)?value:'debounce';save();updateReplyControls();announce('聊天',`回复触发方式已改为${value==='manual'?'连发后手动回复':value==='debounce'?'连发后自动回复':'每条立即回复'}`);
  };

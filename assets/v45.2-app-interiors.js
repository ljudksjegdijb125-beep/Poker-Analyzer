...
(function(){
  ...
  function itemComments(item){item.comments=Array.isArray(item.comments)?item.comments.map(comment=>typeof comment==='string'?{id:'comment_'+v44UUID(),authorType:'user',author:personaFor()?.name||'我',text:comment,at:''}:comment).filter(Boolean):[];return item.comments}
  ...
  function squareShort(state){
    ...
    return`<div class="v452-short-flow">${state.shorts.map(item=>{const comments=itemComments(item),participants=participantTokens(item);return`<article class="v452-short-card">${visualMarkup(item,'生成场景')}<div class="v452-short-actions"><button onclick="toast('作者主页会读取这位人物自己的资料')"><span class="v452-short-avatar">${item.authorType==='user'?'我':'♠'}</span><small>作者</small></button><button class="${item.liked?'on':''}" onclick="v452ToggleSquareLike('short',${A(item.id)})"><span>♥</span><small>${Number(item.likes)||0}</small></button><button onclick="v452OpenSquareComments('short',${A(item.id)})"><span>●</span><small>${comments.length}</small></button><button class="${item.saved?'on':''}" onclick="v452ToggleSquareSave('short',${A(item.id)})"><span>◆</span><small>收藏</small></button><button onclick="toast('分享入口已保留；不会自动外发')"><span>↗</span><small>分享</small></button></div><div class="v452-short-caption"><div><b>@${esc(squareAuthor(item))}</b><button>关注</button></div><p>${esc(item.content||item.summary||'')}</p><section><span>#人物日常</span><span>#此刻想法</span><span>${participants.length?`${participants.length} 人共同参与`:'单人内容'}</span></section><button onclick="v452OpenShortArticle(${A(item.id)})">展开图文与完整内容　›</button>${item.authorType==='user'?`<button class="v452-own-edit" onclick="v452OpenSquareEditor('short',${A(item.id)},'edit')">管理我的内容</button>`:''}</div><small class="v452-swipe-hint">上滑浏览下一条</small></article>`}).join('')}</div>`;
  }
  function squareFeed(state){
    ...
    return`<div class="v452-discover-head"><div><small>DISCOVER</small><h2>图文发现</h2></div><button onclick="v452OpenPublishChoice('feed')">发布</button></div><div class="v452-topic-strip"><button class="on">推荐</button><button>人物日常</button><button>此刻想法</button><button>我的收藏</button></div><div class="v452-discovery-grid">${state.posts.map((item,index)=>`<button class="v452-discover-card tone-${index%3}" onclick="v452OpenPostDetail(${A(item.id)})">${safeImageSrc(item.image)?`<div class="v452-discover-cover has-image"><img src="${attr(safeImageSrc(item.image))}" alt=""><span>${esc(squareAuthor(item))}</span></div>`:`<div class="v452-discover-cover"><span>${esc(squareAuthor(item))}</span><i>◇</i></div>`}<div class="v452-discover-copy"><b>${esc(item.title||item.content)}</b><p>${esc(item.content)}</p><div><span>${item.authorType==='user'?'我':'♠'}</span><small>${esc(squareAuthor(item))}</small><em>♡ ${Number(item.likes)||0}</em></div></div></button>`).join('')}</div>`;
  }
  function squareLong(state){
    return`<section class="v452-long-feature"><div><small>ORIGINAL SERIES</small><h2>长内容与多幕叙事</h2><p>连续生成画面、文字章节、人物旁白和评论共同成立，不伪造播放器。</p><button onclick="v452OpenPublishChoice('long')">生成新内容</button></div><span>◇</span></section><div class="v452-long-filter"><button class="on">推荐</button><button>连载</button><button>收藏</button><button>已读</button></div>${state.longs.length?`<div class="v452-long-list">${state.longs.map(item=>`<button class="v452-long-card" onclick="v452OpenLongDetail(${A(item.id)})">${visualMarkup(item,'多幕内容')}<div><small>${esc(squareAuthor(item))} · ${(item.chapters||[]).length||1} 幕</small><b>${esc(item.title||'未命名长内容')}</b><p>${esc(item.summary||item.content)}</p><span>♡ ${Number(item.likes)||0}　评论 ${itemComments(item).length}<em>完整阅读 ›</em></span></div></button>`).join('')}</div>`:squareEmpty('long')}`;
  }
  ...
  async function generateSquareItem(format,title,direction,participants,wantsImage){
    ...
    try{
      const raw=await invokeModel(modelKind,{system:`你们就是${names}，现在要以各自身份在“广场”发布内容。所有参与者都明确知道这是面向广场受众的发布，不是直接回复${mine}，也不是把聊天内容改写成帖子。根据各自性格、表达习惯、审美、关系状态、当前时间与上下文，选择真正值得公开分享或讨论的有趣侧面；近期相处只用于维持连续性，不得复述对话、汇报主线或机械围绕刚发生的事。不同参与者必须保留各自语气，不得写成同一个编排者的口吻。不得替${mine}发言、行动或表态，不得编造现实账号或现实媒体。内容形态为 ${format}。严格只输出 JSON 对象，键为 title、content、article、imagePrompt、board、chapters。chapters 必须是 1 到 6 个 {"title":"…","text":"…"}；非长内容可为空数组。短内容只描述发布内容本身，不伪造播放器或真实视频文件。`,history:[{role:'user',content:`【当前会话时间】\n${timeContext}\n\n【参与者资料与可用记忆】\n${participantContext(participants,direction)}\n\n【近期相处｜只作背景，不得写成内容摘要】\n${recent||'暂无'}\n\n【${mine}给出的可选方向】\n${direction||'无，由参与者依据自身性格与当下状态决定'}\n\n【可选标题提示】\n${title||'无'}`}],temperature:.82,maxTokens:2200,signal:controller.signal}),parsed=jsonObject(raw),item=contentBase(format,'role',S(parsed.title||title),S(parsed.content||direction||parsed.article),participants);item.article=S(parsed.article||parsed.content||item.content);item.summary=S(parsed.content||parsed.article||item.content);item.board=S(parsed.board||'综合讨论');item.chapters=Array.isArray(parsed.chapters)?parsed.chapters.slice(0,6).map(chapter=>typeof chapter==='string'?{title:chapter,text:''}:{title:S(chapter.title),text:S(chapter.text)}):[];item.imagePrompt=S(parsed.imagePrompt);pushSquareItem(item);
  ...
  window.v452SaveSquareComposer=async function(kind){
    ...
    const wantsImage=document.getElementById('v452GenerateVisual')?.checked===true;v452CloseOverlay();toast('参与者正在构思内容…');try{await generateSquareItem(format,title,text,participants,wantsImage);refreshSquare();toast('内容已进入对应页面')}catch(error){errorDetail(error,'广场内容生成失败')}
  };

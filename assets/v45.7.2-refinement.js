/* =========================================================
   POKEJI V45.7.3 · layered appearance and square stability
   ========================================================= */
(function(){
  'use strict';
  ...
  function syncPalettePreview(){
    ...
    const textInput=document.getElementById('v457PaletteText');if(textInput){textInput.value=contrastText(values.background);textInput.disabled=true;const label=textInput.closest('label')?.querySelector('span');if(label)label.textContent='主要文字 · 自动反色'}
  }
  ...
  function refreshFactoryEntry(){const button=document.querySelector('[data-v471-beauty] button');if(button)button.textContent='分层颜色 / 尺寸 / 实时预览'}
  ...
  function reportSquareError(error){console.error('V45.7.3 square recovery',error);if(typeof errorDetail==='function')errorDetail(error,'广场页面异常已拦截');else if(typeof toast==='function')toast('广场页面数据已修复，请重新打开')}
...
(function(){
  ...
  function repairApiLibrary({persist=false,runLegacy=false}={}){
    ...
    if(runLegacy)try{changed=migrateLegacyModels()||changed}catch(error){console.warn('V45.7.3 legacy API migration skipped',error)}

...
(function(){
  ...
  const labels={...(typeof V435_FUNCTION_LABELS!=='undefined'?V435_FUNCTION_LABELS:{}),characterCompletion:'人物补全'};
  ...
  function draftText(raw){return text(raw).replace(/<state>[\s\S]*?<\/state>/gi,'').replace(/<reverse_phone_request\b[^>]*>[\s\S]*?<\/reverse_phone_request>/gi,'').replace(/<reverse_phone_request\b[^>]*>[\s\S]*$/gi,'').replace(/<reverse_phone_request\b[^>]*\/\s*>/gi,'').replace(/<\/?(?:message|narration|thought|phone_query|phone_update|phone_check|voice|image|sticker)(?:\s+[^>]*)?>/gi,'').trim()||'正在组织回复…'}
  ...
  function composeImagePrompt(prompt,chatId=currentChat,characterOverride=null,personaOverride=null){const ctx=v45CurrentEntity(chatId),character=characterOverride||ctx.character,persona=personaOverride||ctx.persona,parts=[];if(text(data.settings.imagePromptGlobal).trim())parts.push(`【全局绘画规则】\n${text(data.settings.imagePromptGlobal).trim()}`);if(text(character?.imagePrompt).trim())parts.push(`【${character.name||'对方'}的绘画规则】\n${text(character.imagePrompt).trim()}`);if(text(persona?.imagePrompt).trim())parts.push(`【${persona.name||'我的面具'}的绘画规则】\n${text(persona.imagePrompt).trim()}`);if(text(prompt).trim())parts.push(`【本次画面】\n${text(prompt).trim()}`);return parts.join('\n\n')||text(prompt)}
  const baseGenerateImage=generateImageFromProfile;
  window.v45ComposeImagePrompt=composeImagePrompt;
  generateImageFromProfile=function(prompt,context={}){const chatId=context?.chatId||currentChat,baseCtx=v45CurrentEntity(chatId),ctx={character:context?.character||baseCtx.character,persona:context?.persona||baseCtx.persona},finalPrompt=composeImagePrompt(prompt,chatId,ctx.character,ctx.persona);data.runtime.lastImagePromptTrace={at:now(),source:text(context?.source||'chat'),globalApplied:!!text(data.settings.imagePromptGlobal).trim(),characterId:text(ctx.character?.id),characterApplied:!!text(ctx.character?.imagePrompt).trim(),personaId:text(ctx.persona?.id),personaApplied:!!text(ctx.persona?.imagePrompt).trim(),finalPrompt};save();return baseGenerateImage(finalPrompt)};
  ...
  characterBindingPage=function(d){return`${baseCharBinding(d)}<div class="field editor-wide"><label>${esc(d.name||'此人')}的绘画提示词</label><textarea id="char_imagePrompt" placeholder="外貌、固定服装、发型、气质、视觉符号与画风补充">${esc(d.imagePrompt||'')}</textarea><small>只在调用生图时合并，不会改变日常聊天设定。</small></div>`};
  ...
  personaIdentityPage=function(d){return`${basePersonaIdentity(d)}<div class="field editor-wide"><label>${esc(d.name||'这张面具')}的绘画提示词</label><textarea id="persona_imagePrompt" placeholder="这张面具的固定外貌、服装、气质和视觉元素">${esc(d.imagePrompt||'')}</textarea><small>只在生图时使用；不同面具可以拥有不同视觉身份。</small></div>`};
...
const v45StableBaseShowImage=showImageGenerator;
  ...
  v43PostCommit=function(chatId,indexes=[]){
    ...
    const character=directCharacterForChat(chatId);if(!character||data.settings.phoneAutoGenerate!==true||!validAPI()||!indexes?.length)return;
  ...
  window.showChatPlusMenu=function(){
    if(!currentChat)return;const group=groupForChat(currentChat),character=!group&&directCharacterForChat(currentChat),persona=activePersonaFor(currentChat);modal(`<div class="chat-plus-sheet"><div class="chat-plus-title"><small>CHAT TOOLS</small><h2>${group?'群聊工具':esc(character?.name||'聊天工具')}</h2><p>聊天历史仍按正常规则保留；手机只保存站内原创互动资料。</p></div><div class="chat-plus-grid"><button onclick="showStickerPicker()"><span>☺</span><b>表情包</b><small>分类、上传与 URL</small></button><button onclick="showImageGenerator()"><span>✦</span><b>我生成图片</b><small>合并三层绘画提示词</small></button>${group?'':`<button onclick="${currentChatMode==='offline'?`closeModal();openChat(${arg(character.id)},'online')`:`showOfflineEntryChoices(${arg(character.id)})`}"><span>◇</span><b>${currentChatMode==='offline'?'返回线上':'线下相遇'}</b><small>从这里明确切换入口</small></button><button onclick="openSimPhone(${arg(character.id)})"><span>▣</span><b>打开 ${esc(character?.name||'对方')} 的手机</b><small>查看已有内容</small></button><button onclick="closeModal();v45GeneratePhoneNow(${arg(character.id)})"><span>✧</span><b>智能完善 ${esc(character?.name||'对方')} 的手机</b><small>也可以手动触发</small></button>`}<button onclick="openSimPhone('user')"><span>⌁</span><b>打开我的手机</b><small>${esc(persona?.name||'当前面具')}的独立内容</small></button></div></div>`)};
  ...
  const baseSaveCharacter=saveCharacterEditor;saveCharacterEditor=function(...a){const name=characterEditorDraft?.name||'人物';const r=baseSaveCharacter(...a);announce(name,`${name}的资料已保存（含绘画提示词）`);return r};
  const baseSavePersona=savePersonaEditor;savePersonaEditor=function(...a){const name=personaEditorDraft?.name||'面具';const r=baseSavePersona(...a);announce(name,`${name}的资料已保存（含绘画提示词）`);return r};

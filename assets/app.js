...
function applyHomeBackground(){
  ...
  if(!home)return;
  /* V45.7.23 keeps wallpaper data and controls, but the requested global
...
function applyChatBackground(){
  ...
  /* Keep the saved conversation background data intact while honoring the
     V45.7.23 plain-white presentation. */
...
/* V45.7.23：桌面不再挂头像（连右上角那个「我」一起去掉）。
...
function clearCharacterConversations(id){
 /* V45.7.23：清空必须把「会被塞回提示词的痕迹」一起清掉。
...
const V44_SW_URL='/sw-v44.js?build=45.7.23';
...
async function v43FetchWorkerScript(){
 ...
 if(!text.includes("pokeji-v45.7.23"))throw Error('线上 sw-v44.js 仍不是当前版本。请重新覆盖 sw-v44.js，并确认 Vercel 已部署最新 Git 提交。');
...
function v435ExpectedBuild(){return String(V44_SW_URL.match(/[?&]build=([^&#]+)/)?.[1]||'45.7.23')}
async function v43FetchWorkerScript(){
...
function v438TimeContext(chatId=currentChat){const timeline=v438Timeline(chatId);
 ...
 /* V45.7.23：现实时间模式与 v45.4 的时钟保持一致——给一行常识，不给读数。

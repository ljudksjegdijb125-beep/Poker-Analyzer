...
const V44_SW_URL='/sw-v44.js?build=45.2';
...
function showMcpSafetyInfo(){modal(`<h2>本地 MCP 暂未开放</h2><div class="note">公网部署的网站连接 localhost 并不是零风险：网页可能探测或请求本机服务，浏览器还会受本地网络权限与 CORS 限制；一旦允许模型执行工具，也可能被提示注入诱导调用。你的条件是“没有安全风险才加入”，因此 V45.2 没有放入可执行 MCP 工具的入口。后续若加入，只会采用默认关闭、仅 127.0.0.1、工具白名单、每次调用确认且不保存密钥的安全模式。</div><div class="form-actions"><button class="primary" onclick="closeModal()">知道了</button></div>`)}
...
async function checkForUpdates(){
 if(document.body?.dataset.singleFile==='true')return toast('单文件是预览版，请部署 V45.2 资源包更新');
 if(!('serviceWorker' in navigator))return toast('当前浏览器不支持离线更新');
 toast('正在检查更新…');
 try{
  const registration=await ensureV44ServiceWorker({forceUpdate:true});
  if(!registration)throw Error('V45.2 离线服务未能注册');
...
function resetData(){if(!confirm('确定清空 V45.2 的本机数据吗？历史版本的独立存储不会被删除。'))return;try{localStorage.setItem(STORE,JSON.stringify(blank()));localStorage.removeItem(`${STORE}_migration`);location.reload()}catch(error){errorDetail(error,'清空 V45.2 本机数据失败')}}
  ...
  modal(`
    <div class="about-sheet">
      ...
      <div class="about-desc">
        ...
        <p>没有内置角色或聊天。V45.2 只内置两份可查看、可停用、可恢复的线上/线下活人感世界书；其余角色、独立面具聊天、动态、世界、记忆、预设、表情包与虚拟应用均属于本机数据。</p>
      </div>
...
async function v43FetchWorkerScript(){
 const response=await fetch('/sw-v44.js?build=45.2&probe='+Date.now(),{cache:'no-store',credentials:'same-origin'});const type=String(response.headers.get('content-type')||''),text=await response.text();
 ...
 if(!text.includes("pokeji-v45.2"))throw Error('线上 sw-v44.js 仍不是 V45.2。请重新覆盖 sw-v44.js，并确认 Vercel 已部署最新 Git 提交。');
 try{new Function(text)}catch(error){throw Error(`线上 sw-v44.js 语法无效：${error.message}`)}return true;
}
async function checkForUpdates(){
 if(document.body?.dataset.singleFile==='true')return toast('单文件是预览版，请部署 V45.2 资源包更新');if(!('serviceWorker'in navigator))return toast('当前浏览器不支持离线更新');toast('正在检查 V45.2 更新…');
 try{await v43FetchWorkerScript();const registration=await ensureV44ServiceWorker({forceUpdate:true});if(!registration)throw Error('V45.2 离线服务未能注册');if(registration.waiting){registration.waiting.postMessage({type:'SKIP_WAITING'});await waitForWorkerActivation(registration)}toast('V45.2 更新检查完成')}
...
function v435ExpectedBuild(){return String(V44_SW_URL.match(/[?&]build=([^&#]+)/)?.[1]||'45.2')}
async function v43FetchWorkerScript(){
...
async function checkForUpdates(){
 if(document.body?.dataset.singleFile==='true')return toast('单文件是预览版，请部署 V45.2 资源包更新');if(!('serviceWorker'in navigator))return toast('当前浏览器不支持离线更新');toast('正在检查更新…');
...
(function(){
  ...
  exportSJ=function(){
    try{
      ...
      downloadJSON({format:'pokeji-data',version:'45.2',exportedAt:new Date().toISOString(),data:copy},`pokeji-data-${Date.now()}.json`);toast('最终资料已导出（API Key 未包含）');

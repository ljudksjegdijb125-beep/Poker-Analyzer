...
(function(){
  ...
  window.v45CallSend=async function(){
    ...
    const controller=withTimeout(Number(data.settings.timeout)||60000);try{const history=(data.chats[callState.chatId]||[]).filter(m=>m&&!m.phoneEvent&&m.text).slice(-16).map(m=>({role:m.role==='assistant'?'assistant':'user',content:m.text}));history.push({role:'user',content:`【电话中】${value}`});const raw=await invokeModel('chat',{system:`你正在和 USER 进行一通沉浸式电话。你是${character?.name||'角色'}，只说电话里真正会说的话，不要输出旁白、标签、系统说明或“虚拟电话”字样。保持角色语气，回复简短自然。`,history,temperature:data.settings.temperature,maxTokens:Math.min(900,Number(data.settings.maxTokens)||900),signal:controller.signal});const reply=stripReplyTags(raw).trim()||'……';callState.transcript.push({role:'assistant',text:reply});appendPhoneRecord(`电话中：${reply}`,'assistant',{type:'call',direction:'incoming'});saveCall();renderCall()}catch(error){toast(error?.name==='AbortError'?'电话回复已取消':'电话回复失败')}finally{releaseController(controller);callBusy=false}
  };

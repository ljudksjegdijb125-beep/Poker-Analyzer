/* =========================================================
   POKEJI V45.7.10 · incremental fixes and in-place UI additions
...
(function(){
  ...
  window.v45710ClearCharacterBackground=function(characterId){
    try{
      ...
      say('已恢复这条私信的默认纯色背景');
    }catch(error){console.warn('V45.7.10 背景恢复失败',error)}
  };
  ...
  if(baseDictionaryPanel&&!baseDictionaryPanel.__v45710){
    const wrapped=function(state,...rest){
      ...
      let extra='';
      try{extra=linePanelMarkup(learningStore())}catch(error){console.warn('V45.7.10 行式录入渲染失败',error)}

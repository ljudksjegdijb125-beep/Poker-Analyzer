...
async function showTaskNotification(taskId, meta, state) {
  ...
  try {
    await self.registration.showNotification(details.title, {
      ...
      data: {url: APP_ENTRY, chatId: meta.chatId, taskId}
    });
  } catch {}
}
...
self.addEventListener('notificationclick', event => {
  ...
  event.waitUntil((async () => {
    ...
    if (existing) {
      ...
      try { existing.postMessage({type: 'POKEJI_NOTIFICATION_OPEN', chatId: String(event.notification.data?.chatId || '')}); } catch {}
      return;
    }
